...
module AlogramPayRisk
  ...
  class ScoresSuccessResponse < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `AlogramPayRisk::ScoresSuccessResponse` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `AlogramPayRisk::ScoresSuccessResponse`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@next_page_token.nil? && @next_page_token.to_s.length > 4096
        invalid_properties.push('invalid value for "next_page_token", the character length must be smaller than or equal to 4096.')
      end
    ...
    def valid?
      ...
      return false if !@next_page_token.nil? && @next_page_token.to_s.length > 4096
      true
    end
    ...
    def next_page_token=(next_page_token)
      if next_page_token.to_s.length > 4096
        fail ArgumentError, 'invalid value for "next_page_token", the character length must be smaller than or equal to 4096.'
      end
