...
module AlogramPayRisk
  ...
  class Realtime < ApiModelBase
    # Fixed to `realtime` for this schema.
    attr_accessor :type

    attr_accessor :realtime_type
    ...
    def self.openapi_types
      {
        :'type' => :'String',
        :'realtime_type' => :'PaymentRealtimeTypeEnum'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `AlogramPayRisk::Realtime` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `AlogramPayRisk::Realtime`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'realtime_type')
        self.realtime_type = attributes[:'realtime_type']
      end
    end
    ...
    def valid?
      ...
      type_validator = EnumAttributeValidator.new('String', ["realtime"])
    ...
    def type=(type)
      validator = EnumAttributeValidator.new('String', ["realtime"])
