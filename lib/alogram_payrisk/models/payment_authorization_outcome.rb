...
module AlogramPayRisk
  ...
  class PaymentAuthorizationOutcome < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `AlogramPayRisk::PaymentAuthorizationOutcome` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `AlogramPayRisk::PaymentAuthorizationOutcome`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@response_code.nil? && @response_code.to_s.length > 16
        invalid_properties.push('invalid value for "response_code", the character length must be smaller than or equal to 16.')
      end
    ...
    def valid?
      ...
      return false if !@response_code.nil? && @response_code.to_s.length > 16
      true
    end
