...
module AlogramPayRisk
  ...
  class Wallet < ApiModelBase
    ...
    def self.openapi_types
      {
        :'type' => :'String',
        :'wallet_type' => :'PaymentWalletTypeEnum'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `AlogramPayRisk::Wallet` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `AlogramPayRisk::Wallet`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      type_validator = EnumAttributeValidator.new('String', ["wallet"])
    ...
    def type=(type)
      validator = EnumAttributeValidator.new('String', ["wallet"])
