...
module AlogramPayRisk
  class DiscountCode < ApiModelBase
    ...
    attr_accessor :usage_limit_per_user
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `AlogramPayRisk::DiscountCode` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `AlogramPayRisk::DiscountCode`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'usage_limit_per_user')
        self.usage_limit_per_user = attributes[:'usage_limit_per_user']
      end
    end
    ...
    def valid?
      ...
      type_validator = EnumAttributeValidator.new('String', ["percentage", "fixed_amount", "free_shipping", "buy_x_get_y"])
    ...
    def type=(type)
      validator = EnumAttributeValidator.new('String', ["percentage", "fixed_amount", "free_shipping", "buy_x_get_y"])
    ...
    def hash
      [code, amount, type, usage_limit_per_user].hash
    end
