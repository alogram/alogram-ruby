...
module AlogramPayRisk
  class AccountCheckRequest < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `AlogramPayRisk::AccountCheckRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `AlogramPayRisk::AccountCheckRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      event_subtype_validator = EnumAttributeValidator.new('String', ["signup", "login", "password_reset", "mfa_reset", "setting_change"])
    ...
    def event_subtype=(event_subtype)
      validator = EnumAttributeValidator.new('String', ["signup", "login", "password_reset", "mfa_reset", "setting_change"])
    ...
    def hash
      [event_subtype, entities, account, interaction].hash
    end
